...
app.get('/jokes/:id', (req, res) => {
  ...
  res.set('Content-Type', 'application/json')
  res.send(JSON.stringify(specificJoke));
});
...
app.patch('/jokes/:id', (req, res) => {
  ...
  if(!req.body.text) {
    req.body.text = jokes[id - 1].jokeText
  }
  if(!req.body.type) {
    req.body.type = jokes[id - 1].jokeType
  }
  const updatedJoke = {
    id: id,
    jokeText: req.body.text,
  ...
  const jokeToReplaceIndex = jokes.findIndex(joke => joke.id === id)
  jokes[jokeToReplaceIndex] = updatedJoke;
...
app.delete('/jokes/:id', (req, res) => {
  ...
  if (jokeIndex > - 1) {
    ...
    res.sendStatus(404)
    res.json({ error: `Joke with the id ${id} not found` })
  }
}); 
...
app.delete('/all', (req, res) => {
  ...
  if(req.query.key === masterKey) {
    ...
    res
    .sendStatus(200)
    .json('Operação completada com sucesso')
  } else {
    isAuthorized = false
    res.sendStatus(401)
  }
